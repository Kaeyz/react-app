...
`;
function SimilarCard({ id,src,mt, alt, subhead, colorSubHead, detail1, detail2,order1, order2, detail3,flowerRight,flowerLeft, smallsubhead, pad, colorHead, whichImg }) {
  return (
    <Wrapper id={id}>
	 ...
	 <img className="flower-left" src={flowerLeft} />
      ...
      <div className= {`${pad} main-content`}>
        <div className={`${order1} ${mt} first`}>
         {whichImg}
        </div>
        <div className={`${order2} second`}>
          <img src={src} alt={alt} />
          <h2 className={`${colorHead} sub-heading`}>{subhead}</h2>
          <p className="detail1">{detail1}</p>
          <h5 className={`${colorSubHead} smallsubhead`}>{smallsubhead}</h5>
